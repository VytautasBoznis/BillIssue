...
{
    public class RemoveUserAssingmentFromProjectOperation : BaseOperation<RemoveUserAssingmentFromProjectRequest, RemoveUserAssingmentFromProjectResponse>
    {
        private readonly IProjectRepository _projectRepository;

        public RemoveUserAssingmentFromProjectOperation(
            ILogger<RemoveUserAssingmentFromProjectOperation> logger, 
            IUnitOfWorkFactory unitOfWorkFactory, 
            OperationFactory operationFactory, 
            IValidator<RemoveUserAssingmentFromProjectRequest> validator,
            IProjectRepository projectRepository) : base(logger, unitOfWorkFactory, operationFactory, validator)
        {
            _projectRepository = projectRepository;
        }

        protected override async Task<RemoveUserAssingmentFromProjectResponse> Execute(RemoveUserAssingmentFromProjectRequest request, IUnitOfWork unitOfWork)
        {
            ...
            await _projectRepository.DeleteUserAssignmentInTransaction(request.SessionUserData.Id, request.SessionUserData.Email, newProjectAssignmentValues, unitOfWork);

            await unitOfWork.CommitAsync();

            return new RemoveUserAssingmentFromProjectResponse();
        }
    }
}
