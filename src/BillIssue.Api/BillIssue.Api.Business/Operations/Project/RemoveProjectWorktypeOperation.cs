...
{
    public class RemoveProjectWorktypeOperation : BaseOperation<RemoveProjectWorktypeRequest, RemoveProjectWorktypeResponse>
    {
        private readonly IProjectRepository _projectRepository;

        public RemoveProjectWorktypeOperation(
            ILogger<RemoveProjectWorktypeOperation> logger, 
            IUnitOfWorkFactory unitOfWorkFactory, 
            OperationFactory operationFactory, 
            IValidator<RemoveProjectWorktypeRequest> validator,
            IProjectRepository projectRepository) : base(logger, unitOfWorkFactory, operationFactory, validator)
        {
            _projectRepository = projectRepository;
        }

        protected override async Task<RemoveProjectWorktypeResponse> Execute(RemoveProjectWorktypeRequest request, IUnitOfWork unitOfWork)
        {
            ...
            await _projectRepository.MarkProjectWorktypeAsDeletedInTransaction(request.SessionUserData.Id, request.SessionUserData.Email, request.ProjectId, request.ProjectWorktypeId, unitOfWork);

            await unitOfWork.CommitAsync();

            return new RemoveProjectWorktypeResponse();
        }
    }
}
